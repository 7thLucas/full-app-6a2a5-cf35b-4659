...
export async function getConfigReportRecord(configId: string) {
  const config = await findConfigById(configId);
  if (!config) return null;

  const submissions = await JudgmentSubmissionModel.find({ configId: config.pluginId }).sort({ createdAt: -1 });
  const auditLogs = await AuditLogModel.find({ configId: config.pluginId }).sort({ timestamp: -1 }).limit(50);
