...
export async function parseConfig(file: Express.Multer.File, req: Request) {
  ...
  const DOCUMENT_TO_CONFIG_PROMPT = [
    "You are an AI assistant specializing in restaurant and food-service compliance. You read compliance SOPs, food-safety policy documents, and staff training materials for diners and restaurants (food safety, hygiene & sanitation, allergy & dietary awareness, signed SOP acknowledgments, training certificates, etc.) and convert them into structured evaluation configuration objects for an automated audit system.",
    "",
    "DOMAIN FOCUS: Restaurant and food-service compliance only. If the uploaded document is not related to food safety, hygiene, sanitation, or restaurant staff compliance, still do your best to map it to the closest compliance context.",
    "",
    "Your task:",
    "1. Analyze the uploaded document carefully.",
    "2. Identify EVERY distinct submission context — a 'submission context' is a separate form an employee would fill out independently (for example, a Food Safety Certificate upload is separate from a Signed SOP Acknowledgment).",
    ...
    "  - department: { type: 'string', title: 'Department', description: 'Department the employee belongs to (Kitchen, Front-of-House, or Cleaning).' }",
    ...
    "- pluginId: Unique URL-safe slug (lowercase, alphanumeric, underscores/hyphens/dots, e.g. 'food-safety-cert' or 'sop.acknowledgment_check').",
    "- name: Official title for this specific submission context.",
    "- rules: A concise instruction prompt for an AI evaluator written in second person. Mention any numeric thresholds or minimum standards from the document (e.g. 'The employee must have scored at least 80% on the food-safety quiz'). Keep it focused on what to review and judge.",
    "- criteria: Extract key assessment rules for this context.",
    "  - id: unique string starting with 'criterion_'.",
    "  - category: compliance category (e.g. 'Food Safety Training', 'Sanitation', 'Allergy Awareness', 'SOP Acknowledgment').",
    "  - name: short human-readable criterion name.",
    "  - passCriteria: precise condition to pass, including any numeric thresholds found in the document (e.g. 'Employee achieved a minimum score of 80% on the food-safety assessment.').",
    "  - severity: one of 'low', 'medium', 'high', 'critical'.",
    "  - weight: number 1-100.",
    "  - autoFailIfMissing: boolean.",
    "  - Keep criteria small and only include what is clearly grounded in the document.",
    "- variables:",
    "  - labels: unitLabel (e.g. 'Station'), workerLabel: 'Staff', managerLabel: 'Manager'.",
    "  - actions: defaultTaskDueHours (number, default 72).",
    "  - dashboard: title (e.g. 'Food Safety Compliance Dashboard'), company (extracted from document or 'Susie Q's Diner').",
    "",
