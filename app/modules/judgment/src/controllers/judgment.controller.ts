...
export async function getConfigReport(req: Request, res: Response) {
  try {
    const record = await getConfigReportRecord(firstParam(req.params.configId));
    if (!record) {
      return res.status(404).json({ error: "Config not found" });
    }
    return res.json(record);
  } catch (err: any) {
    logger.error("getConfigReport failed", err);
    return res.status(500).json({ error: err.message });
  }
}
